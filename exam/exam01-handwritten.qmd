...
2. When creating scatterplots that include color to indicate a categorical variable, which R function from the tidyverse is primarily used to build the plot framework?
   - (A) `mutate()`
   - (B) `ggplot()`
   - (C) `summarise()`
   - (D) `pivot_longer()`
...
3. Suppose we fit a simple linear regression model to predict monthly apartment rent (in hundreds of dollars) from size (in hundreds of square feet). Which of the following statements about the slope coefficient is **true**?
   - (A) It represents the average change in size for a one-unit increase in rent.
   - (B) It represents the predicted change in rent for a one-unit increase in size.
   - (C) It is guaranteed to be positive whenever there is a positive correlation.
   - (D) It is calculated as the intercept divided by the sample size.
...
4. When conducting a randomization (simulation-based) hypothesis test for a regression slope, which option best describes how the null distribution is generated?
   ...
   - (D) Repeating the original sample slope many times without modification.
...
1. **Partially correct.** The scatterplot may look linear, but an influential outlier can distort the perceived trend. The student should investigate the outlier (e.g., by refitting without it or checking for data issues) before concluding linearity holds.
